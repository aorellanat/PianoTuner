...
if boton_estado == 1
    if(handles.flag1 == 0)
        set(handles.toggleB,'String','Parar grabación');
        sig1 = audiorecorder;
        record(sig1);
...
elseif(boton_estado == 0)
    if(handles.flag1 == 0)
        ...
        handles.fs1 = get(s1, 'TotalSamples');
        sound(handles.signaldata1);
        guidata(hObject,handles);
        set(handles.toggleB,'String','Grabar');
        
        disp("Grabación finalizada");
        axes(handles.axes1);
        plot(handles.signaldata1);
    end
end
...
% new version
[xFF, xAFF] = filterBandPass('New28-v2.wav', 138);
axes(handles.axes2);
plot(xFF,xAFF);
axis([0 500 0 1000])
...
[~, ifqY] = max(xAFF);
frequency = xFF(ifqY);
set(handles.text5,'String', strcat('Frecuencia estándar: ', num2str(frequency), ' [Hz]'));
